...
class KamarController extends Controller
{
    ...
    public function tambah(Request $request)
    {
        ...
        //     'images1.*' => 'nullable|image|max:2048',
        //     'images2.*' => 'nullable|image|max:2048',
        //     'images3.*' => 'nullable|image|max:2048',
        //     'images4.*' => 'nullable|image|max:2048',
        // ]);

        $datakamar = new DataKamar();

        $fileNameFotoPertama = time() . '.' . $request->img_pertama->extension();
        $request->img_pertama->move(public_path('assets/kamar'), $fileNameFotoPertama);

        $fileNameFotoKedua = time() . '.' . $request->img_kedua->extension();
        $request->img_kedua->move(public_path('assets/kamar'), $fileNameFotoKedua);
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($images) {
            foreach (json_decode($datakamar->image) as $image) {
                if (File::exists(public_path('assets/kamar' . $image))) {
                    File::delete(public_path('assets/kamar' . $image));
                }
            }

            foreach ($images as $image) {
                $imageName = time() . '_' . $image->getClientOriginalName();
                $image->move(public_path('assets/kamar'), $imageName);
                $imageNames[] = $imageName;
            }
        }

        $datakamar->image = json_encode($imageNames);
        $datakamar->save();
        return redirect()->route('data-kamar');
    }
